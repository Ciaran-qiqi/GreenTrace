...
export const NFTExchangeCenter: React.FC = () => {
  ...
  }, [isConfirmed, forceRefresh]);

  // 处理错误
  useEffect(() => {
    if (error) {
      console.error('兑换申请错误:', error);
      alert(`兑换申请失败: ${error.message}`);
    }
  }, [error]);
  ...
  const StatusBadge: React.FC<{ status: string }> = ({ status }) => {
    const getStatusConfig = (status: string) => {
      switch (status) {
        ...
        case 'approved':
          return { text: '✅ 已批准', className: 'bg-green-100 text-green-800' };
        case 'rejected':
        ...
        {request.auditComment && (
          <div className="mb-4 p-3 bg-blue-50 rounded-lg border border-blue-200">
            <div className="text-sm text-blue-800">
              <span className="font-medium">审计意见: </span>
      ...
      <div className="max-w-6xl mx-auto">
        <div className="bg-white rounded-xl shadow-lg p-8">
          <div className="text-center py-12">
            ...
            <p className="text-gray-500">连接钱包后查看您的兑换记录</p>
          </div>
        </div>
      </div>
    );
      ...
      <div className="bg-white rounded-xl shadow-lg overflow-hidden">
        <div className="border-b border-gray-200">
          <nav className="-mb-px flex">
            ...
            >
              📋 我的兑换记录 ({userExchangeRequests.length})
            </button>
          </nav>
        </div>
          ...
          {activeTab === 'info' && (
            <div>
              <div className="text-center mb-8">
                <h2 className="text-3xl font-bold text-gray-800 mb-4">NFT兑换说明</h2>
                <p className="text-lg text-gray-600">了解如何将您的绿色NFT兑换为CARB代币</p>
              </div>

              {/* 兑换流程说明 */}
              <div className="bg-gradient-to-br from-blue-50 to-blue-100/50 rounded-xl p-6 mb-8 border border-blue-200">
                <h3 className="text-xl font-semibold text-blue-800 mb-4">兑换流程</h3>
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                  <div className="text-center">
                    ...
                    <h4 className="font-semibold text-blue-800 mb-2">提交申请</h4>
                    <p className="text-sm text-blue-600">在资产页面选择NFT并提交兑换申请</p>
                  </div>
                  <div className="text-center">
                    ...
                    <h4 className="font-semibold text-blue-800 mb-2">等待审计</h4>
                    <p className="text-sm text-blue-600">审计员评估NFT实际价值并确定兑换金额</p>
                  </div>
                  <div className="text-center">
                    ...
                    <h4 className="font-semibold text-blue-800 mb-2">执行兑换</h4>
                    <p className="text-sm text-blue-600">审计通过后，在此页面执行最终兑换</p>
                  </div>
                  <div className="text-center">
                    ...
                    <h4 className="font-semibold text-green-800 mb-2">完成兑换</h4>
                    <p className="text-sm text-green-600">NFT销毁，获得相应的CARB代币</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-gradient-to-br from-yellow-50 to-yellow-100/50 rounded-xl p-6 border border-yellow-200">
                <h3 className="text-xl font-semibold text-yellow-800 mb-4">费用说明</h3>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                  <div className="text-center">
                    <div className="text-2xl mb-2">📝</div>
                    <div className="font-medium text-yellow-800 mb-1">申请手续费</div>
                    <div className="text-yellow-600">碳减排量1%或1个CARB中的较大值</div>
                  </div>
                  <div className="text-center">
                    <div className="text-2xl mb-2">💰</div>
                    <div className="font-medium text-yellow-800 mb-1">系统手续费</div>
                    <div className="text-yellow-600">兑换价值的1%</div>
                  </div>
                  <div className="text-center">
                    <div className="text-2xl mb-2">👨‍💼</div>
                    <div className="font-medium text-yellow-800 mb-1">审计费用</div>
                    <div className="text-yellow-600">兑换价值的4%</div>
                  </div>
                </div>
                <div className="mt-4 p-3 bg-yellow-100/50 rounded-lg">
                  <p className="text-sm text-yellow-700">
                    <strong>最终到账：</strong>审计确认价值的95%（扣除系统手续费和审计费用）
                  </p>
                </div>
              </div>
            </div>
          ...
          {activeTab === 'history' && (
            <div>
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-2xl font-bold text-gray-800">我的兑换记录</h2>
              ...
              {loading ? (
                <div className="text-center py-12">
                  <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600 mx-auto mb-4"></div>
                  <p className="text-gray-600">正在加载兑换记录...</p>
                </div>
              ) : userExchangeRequests.length === 0 ? (
                <div className="text-center py-12">
                  <div className="text-6xl mb-4">🔄</div>
                  <h3 className="text-xl font-semibold text-gray-700 mb-2">暂无兑换记录</h3>
                  <p className="text-gray-500 mb-6">您还没有提交任何NFT兑换申请</p>
                  <button
                    onClick={() => window.location.href = '/assets'}
